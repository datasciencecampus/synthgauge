...
from synthgauge.plot import (plot_correlation, plot_histograms, plot_joint,
                             plot_histogram3d, plot_crosstab)
...
@pytest.mark.parametrize('x, y', [('age', 'height'),
                                  ('age', 'eye_colour'),
...
@pytest.mark.parametrize('feats, method, plot_diff',
                         [(None, 'pearson', False),
                          (None, 'pearson', True),
...
def test_plot_correlation(make_real, make_synth, feats, method, plot_diff):
    ...
    """
    fig = plot_correlation(make_real, make_synth, feats=feats, method=method,
                           plot_diff=plot_diff)
    ...
    if method in ['pearson', 'spearman']:
        expected_feats = make_real[feats].select_dtypes(include='number') \
            .columns
    elif method == 'cramers_v':
        expected_feats = make_real[feats].select_dtypes(exclude='number') \
            .columns
    assert expected_feats.to_list() == x_labels


@pytest.mark.parametrize('feats, method,',
...
@pytest.mark.parametrize('x, y, x_bins, y_bins',
...
@pytest.mark.parametrize('x, y, x_bins, y_bins',
                         [('age', 'height', None, None),
                          ('height', 'eye_colour', None, 'auto')]
                         )
