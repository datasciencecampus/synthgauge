...
def test_wrappers(evaluator):
    ...
    scipy changes.
    """
    evaluator.add_metric('jensen_shannon_distance',
                         feature='hair_colour', bins=None)
    evaluator.add_metric('jensen_shannon_divergence',
                         feature='height', bins='auto')
    ...
    assert (results['jensen_shannon_distance'] ==
            pytest.approx(0.0166117691741195))
    assert (results['jensen_shannon_divergence'] ==
            pytest.approx(0.01942803292479766))
    assert (results['kolmogorov_smirnov'].statistic ==
            pytest.approx(0.088))
    assert (results['kolmogorov_smirnov'].pvalue ==
            pytest.approx(0.0008613642727365059))
    assert (results['kullback_leibler'] ==
            pytest.approx(0.0944474889121178))
    assert (results['mann_whitney'].statistic ==
            pytest.approx(491047.0))
    assert (results['mann_whitney'].pvalue ==
            pytest.approx(0.48780212969583725))
    assert (results['wasserstein'] ==
            pytest.approx(1.8290000000000002))
    assert (results['wilcoxon'].statistic ==
            pytest.approx(203551.0))
    assert (results['wilcoxon'].pvalue ==
            pytest.approx(0.2546321509324163))
    assert (results['kruskal_wallis'].statistic ==
            pytest.approx(0.4814244074541294))
    assert (results['kruskal_wallis'].pvalue ==
            pytest.approx(0.48777782605216824))


def test_classification_metric(evaluator):
    """Check classification metric."""
    evaluator.add_metric('classification_comparison',
                         key=['age', 'height', 'weight', 'hair_colour',
                              'eye_colour'],
                         target='blood_type',
                         sklearn_classifier=RandomForestClassifier,
                         random_state=42)
    result = evaluator.evaluate()
    assert (result['classification_comparison'].precision_difference ==
            pytest.approx(0.1959776822276822))
    assert (result['classification_comparison'].recall_difference ==
            pytest.approx(0.1530778893694964))
    assert (result['classification_comparison'].f1_difference ==
            pytest.approx(0.17732379554135785))
...
def test_propensity_metric_logistic(evaluator):
    ...
    evaluator.add_metric('propensity_metrics', metric_alias='propensity_logrg',
                         method='LogisticRegression', random_state=50,
                         max_iter=10000, solver='sag')
    results = evaluator.evaluate()
    assert (results['propensity_logrg'].observed_p_MSE ==
            pytest.approx(0.014081072437202892))
    assert (results['propensity_logrg'].standardised_p_MSE ==
            pytest.approx(31.523736020704618))
    assert (results['propensity_logrg'].ratio_p_MSE ==
            pytest.approx(10.728436142630775))


def test_propensity_metric_CART(evaluator):
    """Check Propensity metric using CART model."""
    evaluator.add_metric('propensity_metrics', method='CART', random_state=50,
                         num_perms=1000)
    results = evaluator.evaluate()
    assert (results['propensity_metrics'].observed_p_MSE ==
            pytest.approx(0.24841666666666665))
...
def test_TCAP(evaluator):
    """Check TCAP metric."""
    evaluator.add_metric('TCAP', key=['age', 'height', 'weight', 'hair_colour',
                                      'eye_colour'],
                         target='blood_type')
    evaluator.evaluate()
    assert evaluator.metric_results == pytest.approx({
